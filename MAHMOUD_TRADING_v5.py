...
def build_entry(R, alert=False):
    ...
    # 🆕 المستويات الذكية بناءً على السيولة (v5)
    if action != "WAIT":
        # نحاول جلب dataframes للـliquidity analysis
        try:
            from datetime import datetime as _dt
            df_15m_l = R.get("df_15m") or R.get("df")
            df_1h_l = R.get("df_1h")
            df_4h_l = R.get("df_4h")

            # Fallback لو dataframes مش موجودة
            if df_15m_l is None or df_1h_l is None or df_4h_l is None:
                df_15m_l = fetch_tf(sym, "15m", 100)
                df_1h_l = fetch_tf(sym, "1h", 100)
                df_4h_l = fetch_tf(sym, "4h", 100)

            if df_15m_l is not None and df_1h_l is not None and df_4h_l is not None:
                smart_levels = liq_mod.get_smart_levels(
                    action, df_15m_l, df_1h_l, df_4h_l, price
                )

                if smart_levels.get("ok"):
                    m += liq_mod.fmt_smart_levels(smart_levels)

                    # 🤖 AI Verdict (لو AI متاح)
                    if ai_trade.has_any_ai():
                        try:
                            score_data = {
                                "long_score": long_s,
                                "short_score": short_s,
                                ...
                                "mtf": {
                                    ...
                                    "bias_4h": mtf.get("4h", "?"),
                                    "bias_1d": mtf.get("1d", "?"),
                                    "aligned_long": mtf.get("aligned_long", False),
                                    "aligned_short": mtf.get("aligned_short", False),
                                },
                            }
                            verdict = ai_trade.get_ai_verdict(
                                sym, action, score_data, smart_levels, R
                            )
                            m += ai_trade.fmt_ai_verdict(verdict)
                        except Exception as e:
                            logging.warning(f"AI verdict failed: {e}")

                    # دعوة لإضافة الصفقة (مع SL/TP المُفضّل من AI أو Balanced)
                    sl_use = smart_levels["sl"]["balanced"]["level"]
                    tp1_use = smart_levels["tp"]["tp1"]["level"]
                    tp2_use = smart_levels["tp"]["tp2"]["level"]
                    tp3_use = smart_levels["tp"]["tp3"]["level"]
                    action_word = "LONG" if action == "LONG" else "SHORT"
                    m += "\n📋 *لإضافة هذه الصفقة في التتبع:*\n"
                    m += (f"`صفقة {action_word} {sym[:-4]} "
                          f"{fmt(price)} {fmt(sl_use)} "
                          f"{fmt(tp1_use)} {fmt(tp2_use)} "
                          f"{fmt(tp3_use)}`\n\n")
        except Exception as e:
            logging.warning(f"Smart Liquidity levels failed for {sym}: {e}")
            # Fallback للـATR-based القديم
            if R.get("smart_levels"):
                smart = R["smart_levels"]
                bal = smart.get("levels", {}).get("balanced", {})
                m += "🎯 *مستويات (ATR fallback):*\n"
                m += f"📥 الدخول:  `${fmt(price)}`\n"
                m += f"🛑 SL:      `${fmt(bal.get('sl'))}`\n"
                m += f"🎯 TP1:     `${fmt(bal.get('tp1'))}`\n"
                m += f"🎯 TP2:     `${fmt(bal.get('tp2'))}`\n"
                m += f"🎯 TP3:     `${fmt(bal.get('tp3'))}`\n\n"

    # المستويات الذكية القديمة — احتياطي إذا الـLiquidity engine فشل
    elif R.get("smart_levels"):
        smart = R["smart_levels"]
        bal = smart.get("levels", {}).get("balanced", {})
...
async def cmd_start(u: Update, c: ContextTypes.DEFAULT_TYPE):
    ...
    await u.message.reply_text(
        ...
        "💎 *المستويات الذكية (السيولة):*\n"
        "SL × 3 (Conservative/Balanced/Aggressive)\n"
        "TP × 3 (مع احتمالات + Liquidity Map)\n\n"
        f"🤖 AI Verdict: {ai_count}/3 مفعّل "
        f"(C{'✅' if ai_st['claude'] else '❌'} "
        f"G{'✅' if ai_st['gemini'] else '❌'} "
        f"O{'✅' if ai_st['openai'] else '❌'})\n"
        f"⛓ Etherscan: {eth_status} | 🐋 Whale: {whale_st}\n\n"
        "━━━━━━━━━━━━━━━━\n"
        "📈 *تحليل فوري:*\n"
        "أرسل: `BTC` / `ETH` / `SOL` / أي عملة\n"
        "→ يطلع التحليل + Smart SL/TP + AI verdict\n\n"
        "`تابع BTC` — تنبيه عند الإشارات القوية ≥12/15\n\n"
        "━━━━━━━━━━━━━━━━\n"
        "⚡ *Scalping:*\n"
        "`سكالب BTC` — تحليل فوري\n"
        "`تابع سكالب BTC` — متابعة\n"
        "`ماسح_سكالب` — مسح كل العملات كل 5 دقائق\n"
        "`وقف_مسح_سكالب` للإيقاف\n\n"
        "━━━━━━━━━━━━━━━━\n"
        "🔍 *الماسح الذكي (~580 عملة Spot+Futures):*\n"
...
async def handle_msg(u: Update, c: ContextTypes.DEFAULT_TYPE):
    ...
    if text in ("صحة", "health", "تشخيص", "/health"):
        report = ["🏥 *Health Check — v5.0 PURE TRADING*\n"]
        ...
        ai_st = ai_trade.ai_status()
        report.append(f"   AI Verdict keys: Claude={'✅' if ai_st['claude'] else '❌'} "
        ...
        if ai_st['claude']:
            try:
                r = ai_trade.call_claude("Reply: OK", max_tokens=10)
                if r.get("ok"):
                ...
                report.append(f"   Claude: ❌ {type(e).__name__}")

        if ai_st['gemini']:
            try:
                r = ai_trade.call_gemini("Reply: OK", max_tokens=10)
                if r.get("ok"):
                ...
                report.append(f"   Gemini: ❌ {type(e).__name__}")

        if ai_st['openai']:
            try:
                r = ai_trade.call_openai("Reply: OK", max_tokens=10)
                if r.get("ok"):
                ...
                report.append(f"   OpenAI: ❌ {type(e).__name__}")

        if not any([ai_st['claude'], ai_st['gemini'], ai_st['openai']]):
            report.append("   ⚠️ لا توجد AI keys — أضف GEMINI_API_KEY (مجاني)")
        report.append("")
...
async def _post_init(app):
    ...
    print(f"")
    print(f"  AI Verdict (للإشارات): {ai_count}/3")
